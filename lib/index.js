 ...
 * @param  {Object} opts    object of optional params
 */
MoveSteering.prototype.go = function (degrees, speed, turn, opts) {
  var offSpeed = speed * ((100 - (turn * 2)) / 100)
  if (turn === 0) {
    this.motors[0].runDegrees(degrees, {speed: speed, wait: false})
    this.motors[1].runDegrees(degrees, {speed: speed, wait: false})
  } else if (turn > 0) {
    this.motors[0].runDegrees(degrees, {speed: speed, wait: false})
    this.motors[1].runDegrees(degrees, {speed: offSpeed, wait: false})
  } else if (turn < 0) {
    this.motors[0].runDegrees(degrees, {speed: offSpeed, wait: false})
    this.motors[1].runDegrees(degrees, {speed: speed, wait: false})
  }
