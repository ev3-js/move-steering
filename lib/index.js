...
MoveSteering.prototype.forever = function (speed, turn, opts) {
  this.left.forever(speed, defaults(opts, motorDefaults))
  this.right.forever(speed, defaults(opts, motorDefaults))
}
...
MoveSteering.prototype.degrees = function (degrees, speed, turn) {
  ...
  this.left.degrees(degrees, {speed: speeds.left, wait: false})
  this.right.degrees(degrees, {speed: speeds.right, wait: false})
  this.wait()
}
...
MoveSteering.prototyp.wait = function () {
...
function turnToSpeeds (turn, speed) {
  ...
  var reducedSpeed = speed * ((100 - (Math.abs(turn) * 2)) / 100)
